#{
----------------------- Hallar h optimo -------------------------
- ft = valor trucnado de la cpu de la derivada = f + e
1. f'_ex(x0) - f'_aprox(x0) = f'_ex(x0) - (ft(x0 + h) - ft(x0))/h
2. (expando error) = f'_ex(x0) - (f(x0 + h) + e1 - f(x0) - e2)/h
3. (agrupo error) = [f'_ex(x0) - (f(x0 + h) - f(x0))/h] + [(e2 - e1)/h]
4. abs(error cpu) = abs((e2 - e1)/h) <= 2.eps/h (ei <= eps)
5. error = error taylor (depende del orden) + error cpu = h/2.f''(c) + 2.eps/h
6. Minimizamos: f''(c)/2 = 2.eps/h^2 => h = 2.sqrt(eps/f''(c))
#}
