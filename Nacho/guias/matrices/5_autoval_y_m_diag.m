...
- Autovalores: det(A - l.I) = 0; Tantos como dim(A)
- Autovectores: (A - l.I).v = 0
- trace(A) = sum(li)
- det(A) = prod(li)
- Rg(A) = #(l != 0)
- l^n es autovalor de A^n
- Multiplicidad algebraica de autovalor: na(l) = multiplicidad de la raiz
- Multiplicidad geometrica: ng(l) = dimension del subespaci generado
- A diagonalizable <=> na(l) = ng(l)
- Autovectores de autovalores distintos son li
- A simetrica => autovectores de autovalores distintos son ortogonales

-------------------- Matriz Diagonal -----------------------
- Matriz de paso P: autovalores como columnas. Inversible. Ortonormal si A es simetrica
- A_d: matriz diagonal con los autovalores de A. A_d = inv(P).A.P; A = P.A_d.inv(P)
- A_d^n = elevar la diagonal a la n; A^n = P.A_d^n.inv(P)
- Matriz estacionaria: autovalores entre 0 y 1; Matriz estable: autovalores negativos
- Teormea de Forbenius: toda matriz cuyas columnas sumen 1 son estacionarias
- Toda matriz cuadrada, real y simetrica se puede diagonalizar en los reales
#}
