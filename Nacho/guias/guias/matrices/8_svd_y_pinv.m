...
- pinv(A).A = I
- row > col: pinv(A) = inv(A'.A).A' (A debe ser inversible)
- col > row: pinv(A) = A'.inv(A.A') (A debe ser inversible)

------------- Descomposicion Valores Singulares (SVD) ------------
- A'.A es simetrica y autovalores >= 0
- Valor singular (o): raiz cuadrada positiva de los autovalores de A'.A de mayor a menor
- A = USV'; A[m,n]; rotacion.deformacion.rotacion
- Completa: U[m,m] ortonormal, S[m,n] diagonal con 0, V[n,n] ortonormal
- Reducida: U[m,l] ortonormal, S[l,l] diagonal sin 0, V[n,l] ortonormal con l #v.s.
- S son los valores singulares ordenados, y luego ceros (completa)
- V son los autovectores de A'A normalizados ordenados
- Ui = A.vi/oi
- pinv(A) = V.inv(S).U'
- Norma 2: max(oi)
- det(A) = det(S) = prod(oi)
